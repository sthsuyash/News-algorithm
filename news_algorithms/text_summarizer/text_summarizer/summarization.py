...
class Summarizer:
    def __init__(self, *args, **kwargs):
        """
        Initializes the Summarize class by loading the word embeddings.
    ...
    def generate_centroid_tfidf(self, sentences: List[str]) -> np.ndarray:
        ...
        important_terms = [
            ...
            if word in self.word_vec.key_to_index and tfidf_normalized[i] >= 0.2
        ]
        centroid = np.mean([
            self.word_vec[word]
            for word in important_terms
        ], axis=0) if important_terms else np.zeros(self.vector_size)
    ...
    def sentence_vectorizer(self, sentences: List[str]) -> Dict[int, np.ndarray]:
        ...
        for i, sentence in enumerate(sentences):
            ...
            word_count = 0
            for word in words:
                if word in self.word_vec.key_to_index:
    ...
    def sentence_selection(self, centroid: np.ndarray, sentence_vectors: Dict[int, np.ndarray], summary_length: int) -> List[int]:
        ...
        """
        similarities = []
        for sentence_id, sentence_vec in sentence_vectors.items():
            similarity = 1 - cosine(centroid, sentence_vec)
            similarities.append((sentence_id, similarity))
    ...
    def show_summary(self, text: str, length_sentence_predict: float = 0.01) -> str:
        ...
        Args:
            ...
            length_sentence_predict (float): A float between 0 and 1 indicating the desired length of the summary as a proportion
        ...
        # Generate centroid of important terms
