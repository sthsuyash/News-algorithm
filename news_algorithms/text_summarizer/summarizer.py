...
from news_algorithms.utils.preprocess import preprocess
from news_algorithms.utils.embedding import Embeddings
...
class Summarizer:
    ...
    def generate_centroid_tfidf(self, sentences: List[str]) -> np.ndarray:
        ...
        centroid = np.mean([self.word_vec[word]
                           for word in important_terms], axis=0)
        return centroid
